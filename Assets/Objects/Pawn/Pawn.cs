...
namespace Game
{
    public class Pawn : MonoBehaviourPun
    {
        ...
        public QuestionManager QuestionManager { get { return Core.QuestionManager; } } // Tambahan dependancy QuestionManager
        ...
        // Fungsi baru untuk menjawab pertanyaan
        public void AnswerQuestion(int selectedIndex)
        {
            var question = QuestionManager.GetRandomQuestion();
            if (question != null)
            {
                bool isCorrect = question.CorrectAnswerIndex == selectedIndex;
                if (isCorrect)
                {
                    Core.AddScore(10);  // Tambahkan skor jika jawaban benar
                }
            }
        }
